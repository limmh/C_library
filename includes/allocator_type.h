...
/**
 * @brief An allocator interface which supports user-defined allocation, reallocation and deallocation functions.
 *
...
/**
 * @brief Initializes the function pointers of an allocator.
 * @param [in, out] allocator A pointer to an allocator object, must not be null
 * @param [in] allocation_function_pointer A pointer to a user-defined memory allocation function, must not be null
 * @param [in] reallocation_function_pointer A pointer to a user-defined memory reallocation function, can be null
 * @param [in] deallocation_function_pointer A pointer to a user-defined memory deallocation function, must not be null
 */
INLINE_OR_STATIC
...
/**
 * @brief Clears the function pointers of an allocator by setting each of them to null.
 * @param allocator A pointer to an allocator object (MUST NOT be NULL)
 */
INLINE_OR_STATIC
...
/**
 * @brief Performs memory allocation by calling the 'allocate' function pointer.
 * @param [in] allocator An allocator object passed by value (its 'allocate' and 'deallocate' function pointers must be valid and non-null)
 * @param [in] number_of_bytes: Number of bytes of memory to be allocated
 * @return void * A pointer to a memory block if allocation is successful, otherwise NULL
 *
 * Notes:
 ...
 * - The user-defined allocation function should return a null pointer if no memory block of the requested size can be allocated.
...
/**
 * @brief Performs memory reallocation by calling the 'reallocate' function pointer.
 * @param [in] allocator An allocator object passed by value, its 'allocate' functions and 'deallocate' functions must be valid and non-null
 * @param [in] old_block MUST point to a memory block previously allocated by the 'allocate' function
 * @param [in] old_number_of_bytes The number of bytes previously passed to 'allocate' to obtain old_block
 * @param [in] new_number_of_bytes The number of bytes of memory to reallocate, can be smaller than, the same as or greater than old_number_of_bytes
 * @return void * a pointer to a new memory block if reallocation is successful, otherwise NULL
 *
 * Notes:
...
/**
 * @brief Deallocates an allocated memory block by calling the 'deallocate' function pointer.
 * @param [in] allocator An allocator object passed by value, its 'allocate' and 'deallocate' function pointers must be valid and non-null
 * @param [in] memory_block MUST point to a memory block previously allocated by the 'allocate' or 'reallocate' function,
